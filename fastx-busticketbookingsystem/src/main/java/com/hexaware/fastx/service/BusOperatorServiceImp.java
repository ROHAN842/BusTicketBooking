...
//	}
	...
	@Override
	public BusSchedule editBusSchedule(BusScheduleDTO busScheduleDto, int scheduleId) throws ScheduleNotFoundException {
	    Optional<BusSchedule> existingScheduleOptional = busScheduleRepo.findById(scheduleId);
	    
	    if(existingScheduleOptional.isPresent()) {
	        BusSchedule existingSchedule = existingScheduleOptional.get();
	        
	        // Retrieve the existing BusOperator from the database
	        Optional<BusOperator> existingOperatorOptional = busOperatorRepo.findById(busScheduleDto.getOperatorId());
	        
	        if(existingOperatorOptional.isPresent()) {
	            BusOperator existingOperator = existingOperatorOptional.get();
	            
	            // Update the existing BusSchedule with the new values
	            existingSchedule.setBusNumber(busScheduleDto.getBusNumber());
	            existingSchedule.setAvailableSeats(busScheduleDto.getAvailableSeats());
	            existingSchedule.setDate(busScheduleDto.getDate());
	            existingSchedule.setTimings(busScheduleDto.getTimings());
	            existingSchedule.setStatus(busScheduleDto.getStatus());
	            existingSchedule.setAmenities(busScheduleDto.getAmenities());
	            existingSchedule.setBusType(busScheduleDto.getBusType());
	            existingSchedule.getBusRoute().setRouteID(busScheduleDto.getRouteId());
	            existingSchedule.setOperator(existingOperator); // Set the existing operator
	            
	            // Save the updated BusSchedule
	            return busScheduleRepo.save(existingSchedule);
	        } else {
	            throw new ScheduleNotFoundException("Operator with the given ID not found in DB.");
	        }
	    } else {
	        throw new ScheduleNotFoundException("Schedule with the given ID not found in DB.");
	    }
